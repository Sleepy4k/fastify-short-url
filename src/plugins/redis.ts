...
export default fp(
  async function redisPlugin(app: FastifyInstance) {
    ...
    const redis = new Redis({
      ...
      keyPrefix: app.config.REDIS_KEY_PREFIX || undefined,
      tls: app.config.REDIS_TLS === "true" ? {} : undefined,
      family: app.config.REDIS_FAMILY,
      connectTimeout: app.config.REDIS_CONNECT_TIMEOUT,
      commandTimeout: app.config.REDIS_COMMAND_TIMEOUT,
